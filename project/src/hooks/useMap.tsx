...
// Хук useState добавит состояние для пользовательского хука
...
export default function useMap(mapRef: MutableRefObject<HTMLElement | null>, city: City) {
  ...
  useEffect(() => {
    // Проверка на первую отрисовку карты?
    if (mapRef.current !== null && !isRenderedRef.current) {
      // new Map - создает карту
      // Map: 1й аргумент - ссылка на HTML-элемент (куда отрендерить карту)
      // Map: 2й аргумент - объект доп. параметров. Мин. набор доп. параметров - координаты центра карты и коэффициент масштабирования
      ...
      // TileLayer - позволяет подключить определенный слой карты
      // Мы взяли слой voyager
